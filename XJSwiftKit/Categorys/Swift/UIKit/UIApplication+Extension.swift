...
extension UIApplication: XJCompatible {}
// MARK:- 一、基本的扩展
public extension XJExtension where Base: UIApplication {
    ...
    static var userAgent: String {
        ...
        return "JK" + Bundle.xj.appVersion
    }
    
    // MARK: 1.4、app定位区域
    /// app定位区域
    static var localizations: String? {
        guard let weakInfoDictionary = Bundle.xj.infoDictionary, let content = weakInfoDictionary[String(kCFBundleLocalizationsKey)] else {
            return nil
        }
...
public extension XJExtension where Base: UIApplication {
    ...
    static func openSystemApp(type: JKSystemAppType, complete: @escaping ((Bool) -> Void)) {
        openUrl(url: URL(string: type.rawValue)!, complete: complete)
    }
