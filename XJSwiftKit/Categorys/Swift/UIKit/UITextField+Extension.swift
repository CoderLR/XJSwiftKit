...
// MARK: - Create
public extension XJExtension where Base: UITextField {
    ...
    ///   - tintColor: 光标颜色
    ///   - font: 字体大小
    ///   - isSecureTextEntry: *显示
    ///   - clearButtonMode: x显示
    /// - Returns: UITextField
    static func create(bgColor: UIColor = UIColor.white,
                       borderStyle: Base.BorderStyle = .none,
                       placeholder: String? = nil,
                       textColor: UIColor = Color_333333_333333,
                       tintColor: UIColor = Color_System,
                       font: CGFloat = 14,
                       isSecureTextEntry: Bool = false,
                       clearButtonMode: UITextField.ViewMode = .whileEditing) -> UITextField {
        let textfield = UITextField()
        textfield.backgroundColor = bgColor
        textfield.borderStyle = borderStyle // 样式
        if let placeholder = placeholder {
            textfield.placeholder = placeholder // placeholder内容
        }
        textfield.textColor = textColor // 字体颜色
        textfield.tintColor = tintColor // 光标颜色
        textfield.font = UIFont.systemFont(ofSize: font) // 字体大小
        textfield.isSecureTextEntry = isSecureTextEntry // 密码显示
        textfield.clearButtonMode = clearButtonMode // X按钮
        /*
        textfield.setValue(Color_System, forKey: "_placeholderLabel.textColor")
        textfield.returnKeyType = .done // 发送按钮
        textfield.leftViewMode = .always // leftView显示方式
        */
        return textfield
    }
