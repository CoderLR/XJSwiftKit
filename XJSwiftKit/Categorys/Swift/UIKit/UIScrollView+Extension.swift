...
// MARK: - Create
public extension XJExtension where Base: UIScrollView {
    
    /// UIScrollView创建
    /// - Parameters: - bgColor: 背景颜色
    /// - Returns: UIScrollView
    static func create(bgColor: UIColor = UIColor.white) -> UIScrollView {
    ...
    private func snapShotContentScrollPage(index: Int, maxIndex: Int, callback: @escaping () -> Void) {
        ...
        DispatchQueue.xj.asyncDelay(Double(Int64(0.3 * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)) {
        } _: {
