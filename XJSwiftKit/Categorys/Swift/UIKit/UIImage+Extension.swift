...
extension UIImage: XJCompatible {}
// MARK:- 一、基本的扩展
public extension XJExtension where Base: UIImage {
    ...
    static func create(color: UIColor) -> UIImage {
        let image = Base.xj.create(color: color, size: CGSize(width: 1, height: 1))
        return image
    }
    ...
    static func textImage(_ text: String, fontSize: CGFloat = 16, size: (CGFloat, CGFloat), backgroundColor: UIColor = UIColor.orange, textColor: UIColor = UIColor.white, isCircle: Bool = true, isFirstChar: Bool = false) -> UIImage? {
        ...
        let textsize = text.xj.rectSize(font: UIFont.systemFont(ofSize: fontSize), size: CGSize(width: KScreenW, height: CGFloat(MAXFLOAT)))
    ...
    func solidTo(size: CGSize) -> UIImage? {
        ...
        if self.base.size.height <= self.base.size.width {
            if self.base.size.width >= w {
                let scaleImage = fixOrientation().xj.scaleTo(size: CGSize(width: w, height: w * self.base.size.height / self.base.size.width))
                return scaleImage
            } else {
            ...
            if self.base.size.height >= h {
                let scaleImage = fixOrientation().xj.scaleTo(size: CGSize(width: h * self.base.size.width / self.base.size.height, height: h))
                return scaleImage
            } else {
                return fixOrientation()
            }
...
public extension XJExtension where Base: UIImage {
    ...
    func compress(mode: CompressionMode = .medium) -> Data? {
        return resizeIO(resizeSize: mode.resize(base.size))?.xj.compressDataSize(maxSize: mode.maxDataSize)
    }
    ...
    func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@escaping (Data?, CGSize) -> Void) {
        queue.async {
            let data = resizeIO(resizeSize: mode.resize(self.base.size))?.xj.compressDataSize(maxSize: mode.maxDataSize)
            DispatchQueue.main.async {
...
public extension XJExtension where Base: UIImage {
    ...
    private static func getHDImgWithCIImage(with image: CIImage, size: CGSize, isLogo: Bool = true, logoSize: CGSize?, logoImage: UIImage? = nil, logoRoundCorner: CGFloat? = nil) -> UIImage? {
        ...
        if let newLogoRoundCorner = logoRoundCorner, let roundCornerLogo = logoImage.xj.isRoundCorner(radius: newLogoRoundCorner, byRoundingCorners: .allCorners, imageSize: logoSize) {
