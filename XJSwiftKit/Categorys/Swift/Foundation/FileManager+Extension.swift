...
extension FileManager: XJCompatible {}
// MARK:- 一、沙盒路径的获取
/*
 - 1、Home(应用程序包)目录
...
public extension XJExtension where Base: FileManager {
...
public extension XJExtension where Base: FileManager {
    ...
    static func readFromFile(readType: FileWriteType, readPath: String) -> (isSuccess: Bool, content: Any?, error: String) {
        ...
        case .ArrayType:
            ...
            return (true, readString.xj.jsonStringToArray(), "")
        case .DictionaryType:
            guard let readString = String(data: data, encoding: String.Encoding.utf8) else {
                return (false, nil, "读取内容失败")
            }
            return (true, readString.xj.jsonStringToDictionary(), "")
        }
    }
