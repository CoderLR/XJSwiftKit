...
extension String: XJCompatible {}
//ExpressibleByStringLiteral
// MARK:- 一：字符串基本的扩展
public extension XJExtension where Base == String {
    ...
    func toClass() -> AnyClass? {
        ...
        guard let Class: AnyClass = NSClassFromString(namespace.xj.removeSomeStringUseSomeString(removeString: " ", replacingString: "_") + "." + base) else {
            return nil
        }
        return Class
    }
    ...
    func separatedByString(with char: String) -> Array<Any> {
        return base.components(separatedBy: char)
    }
    ...
    func setHtmlAttributedString(font: UIFont? = UIFont.systemFont(ofSize: 16), lineSpacing: CGFloat? = 10) -> NSMutableAttributedString {
        ...
        do {
            if let data = self.base.replacingOccurrences(of: "\n", with: "<br/>").data(using: .utf8) {
        ...
        return htmlString ?? NSMutableAttributedString(string: self.base)
    }
 ...
 */
public extension XJExtension where Base: ExpressibleByStringLiteral {
...
public extension XJExtension where Base: ExpressibleByStringLiteral {
    ...
    var toNumber: NSNumber? {
        return self.toDouble()?.xj.number
    }
}

// MARK:- 五、字符串UI的处理
extension XJExtension where Base: ExpressibleByStringLiteral {
...
extension XJExtension where Base: ExpressibleByStringLiteral {
    ...
    public var isValidIDCardNumStrict: Bool {
        ...
        case 15:
            ...
            year = Int(str.xj.sub(start: 6, length: 2))!
        ...
        case 18:
            // 18位身份证
            year = Int(str.xj.sub(start: 6, length: 4))!
            ...
            if numberOfMatch > 0 {
                ...
                for i in 0 ..< 17 {
                    if let d = Int(str.xj.slice(i ..< (i + 1))) {
                        s += d * jiaoYan[i % 10]
                    } else {
                        return false
                    }
                ...
                let Y = s % 11
                let JYM = "10X98765432"
                let M = JYM.xj.sub(start: Y, length: 1)
                if M == str.xj.sub(start: 17, length: 1) {
                    return true
                } else {
                    return false
                }
...
extension XJExtension where Base: ExpressibleByStringLiteral {
    ...
    public func nsRange(of subString: String) -> NSRange? {
        guard (base as! String).xj.contains(find: subString) else {
            return nil
        }
    ...
    public func insertString(content: String, locat: Int) -> String {
        ...
        let str1 = (base as! String).xj.sub(to: locat)
        let str2 =  (base as! String).xj.sub(from: locat + 1)
...
public extension XJExtension where Base: ExpressibleByStringLiteral {
    ...
    func binaryToOctal() -> String {
        ...
        // 八进制
        return decimal.xj.decimalToOctal()
    }
    ...
    func binaryToHexadecimal() -> String {
        ...
        let decimal = binary.binaryTodecimal()
        // 十六进制
        return decimal.xj.decimalToHexadecimal()
    }
    ...
    func octalTobinary() -> String {
        ...
        // 二进制
        return decimal.xj.decimalToBinary()
    }
    ...
    func octalToHexadecimal() -> String {
        ...
        // 十进制
        let decimal = octal.xj.octalTodecimal()
        // 十六进制
        return decimal.xj.decimalToHexadecimal()
    }
    ...
    func decimalToBinary() -> String {
        guard var decimal = (self.base as! String).xj.toInt() else {
            return ""
        }
        var str = ""
        while decimal > 0 {
    ...
    func decimalToOctal() -> String {
        guard let decimal = (self.base as! String).xj.toInt() else {
            return ""
        }
        /*
