...
// MARK:- 一、Date 基本的扩展
public extension XJExtension where Base == Date {
...
public extension XJExtension where Base == Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampInt = timestamp.xj.toInt() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
...
public extension XJExtension where Base == Date {
    ...
    var isYesterday: Bool {
        // 1.先拿到昨天的 date
        guard let date = Base.xj.yesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
    ...
    var isTheDayBeforeYesterday: Bool  {
        // 1.先拿到前天的 date
        guard let date = Base.xj.theDayBeforYesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
