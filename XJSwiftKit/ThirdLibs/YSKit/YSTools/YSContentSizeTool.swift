...
// MARK:- 一、文字内容的计算
public struct YSContentSizeTool {
...
// MARK:- 二、文本行数和内容的计算
public extension YSContentSizeTool {
    // MARK: 2.1、行数和每行的内容
    ...
    static func linesCountAndLinesContent(content: String, font: UIFont, width: CGFloat, height: CGFloat, minimumScaleFactor: CGFloat = 1.0) -> (Int?, [String]?) {
        ...
        for line in lines {
            ...
            let lineString = content.xj.sub(start: lineRange.location, length: lineRange.length)
