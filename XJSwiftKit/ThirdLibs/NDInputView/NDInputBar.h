 /******************************************************************************
  *
  *  本文件声明了 NDTextViewDelegate 协议和 NDInputBar 类。
  *  TUI 输入条，用于检测、获取用户输入的 UI 组件。
  *  NDInputBar，即位于聊天消息最下方的 UI 组件。包括文本输入框、表情按钮、语音按钮和“+”按钮（“更多”按钮）。
  *  NDTextViewDelegate 提供了输入条各种情况下的回调委托，包括点击输入条的表情、“更多”视图、语音按钮的回调。以及发送消息、发送语音、更改输入高度的回调。
  *
  ******************************************************************************/
 ...
 */
@protocol NDTextViewDelegate <NSObject>
/**
 *  点击表情按钮，即“笑脸”后的回调委托。
 ...
 *  在 ND 默认的实现中，本回调函数在处理了表情视图与更多视图的浮现后，进一步调用了 TUIInputController 中的 didChangeHeight 委托进行 UI 布局的高度调整。
 *
 *  @param textView 委托者，当前与用户交互的 InputBar。
 ...
 *  在 ND 默认的实现中，本回调函数在处理了表情视图与更多视图的浮现后，进一步调用了 TUIInputController 中的 didSendMessage 委托进行消息发送的进一步逻辑处理。
 *
 *  @param textView 委托者，当前与用户交互的 InputBar。
 ...
 *  继承自 UITextView
 */
@property (nonatomic, strong) NDResponderTextView *inputTextView;
...
/**
 *  实现 NDTextViewDelegate 协议的委托。
 */
@property (nonatomic, weak) id<NDTextViewDelegate> delegate;

/**
*  设定输入方式
*
*  @param status 类型（YES：语音输入，NO：文字输入）
*/
