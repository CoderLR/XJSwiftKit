 ...
 *  NDMenuView 类，即表情菜单视图“本体”，负责在 UI 中以视图的形式进行显示，同时作为盛放各个组件的“容器”。
 ...
 *  在 ND 的默认实现中，委托调用链为 menuView -> inputController -> messageController。
 *  分别调用上述类中的 sendMessage 委托/函数，使得功能合理分层的同时提高代码复用率。
 *  上述实现仅为您对该委托的实现提供参考作用，帮助您更好的理解 ND 的运行机制。
 *
 *  @param menuView 委托者，表情菜单视图
 */
