...
@interface NDInputController () <NDTextViewDelegate, TMenuViewDelegate, TFaceViewDelegate, TMoreViewDelegate>
@end

@implementation NDInputController
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self setupViews];
...
- (void)setupViews
{
    ...
    _inputBar = [[NDInputBar alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, TTextView_Height)];
    _inputBar.delegate = self;
    [self.view addSubview:_inputBar];
}

- (void)setStatus:(InputStatus)status {
    _status = status;
    if (_status == Input_Status_Input_Talk) {

        [_inputBar setupVoiceOrTextInput:YES];
    }
}
...
- (void)inputBarDidTouchVoice:(NDInputBar *)textView
{
    if(_status == Input_Status_Input_Talk){
        return;
    }
...
- (void)inputBarDidTouchMore:(NDInputBar *)textView
{
...
- (void)inputBarDidTouchFace:(NDInputBar *)textView
{
...
- (void)inputBarDidTouchKeyboard:(NDInputBar *)textView
{
    if(_status == Input_Status_Input_More){
        [self hideMoreAnimation];
    }
...
- (void)inputBar:(NDInputBar *)textView didChangeInputHeight:(CGFloat)offset
{
    if(_status == Input_Status_Input_Face){
        [self showFaceAnimation];
    }
...
- (void)inputBar:(NDInputBar *)textView didSendText:(NSString *)text
{
...
- (void)inputBar:(NDInputBar *)textView didSendVoice:(NSString *)path
{
    /*
...
- (void)menuView:(NDMenuView *)menuView didSelectItemAtIndex:(NSInteger)index
{
    [self.faceView scrollToFaceGroupIndex:index];
}

- (void)menuViewDidSendMessage:(NDMenuView *)menuView
{
...
- (void)faceView:(NDFaceView *)faceView scrollToFaceGroupIndex:(NSInteger)index
{
    [self.menuView scrollToMenuIndex:index];
}

- (void)faceViewDidBackDelete:(NDFaceView *)faceView
{
    [_inputBar backDelete];
}

- (void)faceView:(NDFaceView *)faceView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    TFaceGroup *group = [NDConfig defaultConfig].faceGroups[indexPath.section];
    ...
    if(indexPath.section == 0){
        ...
        if(_delegate && [_delegate respondsToSelector:@selector(inputController:didSendMessage:)]){
            [_delegate inputController:self didSendMessage:@"didSelectItemAtIndexPath"];
        }
    }
}

#pragma mark - more view delegate
- (void)moreView:(NDMoreView *)moreView didSelectMoreCell:(NDInputMoreCell *)cell
{
...
{
    if(!_faceView){
        _faceView = [[NDFaceView alloc] initWithFrame:CGRectMake(0, _inputBar.frame.origin.y + _inputBar.frame.size.height, self.view.frame.size.width, TFaceView_Height)];
        _faceView.delegate = self;
        [_faceView setData:[NSMutableArray arrayWithArray:[NDConfig defaultConfig].faceGroups]];
    }
    return _faceView;
}
...
{
    if(!_moreView){
        _moreView = [[NDMoreView alloc] initWithFrame:CGRectMake(0, _inputBar.frame.origin.y + _inputBar.frame.size.height, self.faceView.frame.size.width, 0)];
...
{
    if(!_menuView){
        _menuView = [[NDMenuView alloc] initWithFrame:CGRectMake(0, self.faceView.frame.origin.y + self.faceView.frame.size.height, self.view.frame.size.width, TMenuView_Menu_Height)];
        _menuView.delegate = self;

        NDConfig *config = [NDConfig defaultConfig];
...
- (void)setIndexPath_ContentCell:(NSIndexPath *)indexPath_ContentCell {
    _indexPath_ContentCell = indexPath_ContentCell;
}

- (void)setIndexPath_DiscussCell:(NSIndexPath *)indexPath_DiscussCell {
    _indexPath_DiscussCell = indexPath_DiscussCell;
}
